    ...
    <div class="bg-white shadow-sm rounded-xl border border-gray-200 mt-4">
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between px-4 sm:px-6 py-2.5 border-b border-gray-100">
            <div class="flex items-center gap-2 min-w-0">
                ...
                <div>
                    <h4 class="text-sm font-bold text-gray-900 leading-tight">Edit Money Exchange #{{ $moneyExchange->money_exchange_id }}</h4>
        ...
        <form action="{{ route('money-exchanges.update', $moneyExchange) }}" method="POST" id="exchangeForm" enctype="multipart/form-data" class="px-4 sm:px-6 py-2 pb-3 space-y-1.5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-1.5 pb-1.5 border-b border-gray-100">
                    ...
                    <div class="flex items-center gap-4 h-9">
                        <label class="inline-flex items-center gap-1.5 cursor-pointer">
                            <input type="radio" name="transaction_operation" value="1"
                                {{ old('transaction_operation', $moneyExchange->transaction_operation ?? 2) == '1' ? 'checked' : '' }}
                        ...
                        <label class="inline-flex items-center gap-1.5 cursor-pointer">
                            <input type="radio" name="transaction_operation" value="2"
                                {{ old('transaction_operation', $moneyExchange->transaction_operation ?? 2) == '2' ? 'checked' : '' }}
                                class="border-gray-300 text-indigo-600 focus:ring-indigo-500"
                                onchange="onOperationChange()" />
                ...
                <div>
                    ...
                    <input type="number" id="rate" name="rate" step="any" min="0.0001" value="{{ old('rate', $moneyExchange->rate) }}" required inputmode="decimal"
            ...
            <details class="border border-gray-200 rounded-lg bg-gray-50 group" {{ $moneyExchange->attachments->count() > 0 ? 'open' : '' }}>
                <summary class="flex items-center gap-2 px-4 py-2.5 cursor-pointer list-none select-none">
                    ...
                    @if($moneyExchange->attachments->count() > 0)
                        <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700">{{ $moneyExchange->attachments->count() }}</span>
                    @endif
