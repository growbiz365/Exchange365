<x-app-layout>
    @section('title', 'Bank Transfer #' . $bankTransfer->bank_transfer_id . ' - ExchangeHub')
    <x-breadcrumb :breadcrumbs="[
        ['url' => route('dashboard'), 'label' => 'Home'],
        ['url' => route('banks.index'), 'label' => 'Banks'],
        ['url' => route('bank-transfers.index'), 'label' => 'Bank Transfers'],
        ['url' => route('bank-transfers.show', $bankTransfer), 'label' => 'Transfer #' . $bankTransfer->bank_transfer_id]
    ]" />
        ...
        <div class="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="flex items-start gap-3 sm:space-x-4 min-w-0">
                ...
                <div class="min-w-0 flex-1">
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">Bank Transfer #{{ $bankTransfer->bank_transfer_id }}</h1>
                    <p class="text-xs sm:text-sm text-gray-500 mt-0.5">
                        {{ $bankTransfer->date_added->format('l, d F Y') }}
                        @if($bankTransfer->user)
                            <span class="text-gray-400"> · Recorded by {{ $bankTransfer->user->name }}</span>
                        @endif
                    </p>
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 w-full lg:w-auto lg:shrink-0">
                <a href="{{ route('bank-transfers.print', $bankTransfer) }}" target="_blank"
                ...
                <form action="{{ route('bank-transfers.destroy', $bankTransfer) }}" method="POST" class="w-full sm:w-auto"
                ...
                <a href="{{ route('bank-transfers.index') }}"
                    class="inline-flex justify-center items-center rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50 transition focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 w-full sm:w-auto sm:col-span-2 lg:col-span-1">
                    <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-4 sm:p-6">
                ...
                <dl class="space-y-4">
                    <div>
                        ...
                        <dd class="mt-1 text-base font-semibold text-gray-900">{{ $bankTransfer->fromBank?->bank_name ?? '—' }}</dd>
                    </div>
                    <div>
                        ...
                        <dd class="mt-1 text-sm font-medium text-gray-900">
                            ...
                            <span class="text-gray-500">({{ $bankTransfer->fromBank?->currency?->currency_symbol ?? '—' }})</span>
                        </dd>
                    </div>
                </dl>
            </div>
            ...
            <div class="p-4 sm:p-6">
                ...
                <dl class="space-y-4">
                    <div>
                        ...
                        <dd class="mt-1 text-base font-semibold text-gray-900">{{ $bankTransfer->date_added->format('d M Y') }}</dd>
                    </div>
                    <div>
                        ...
                        <dd class="mt-1">
                            <span class="inline-flex items-center rounded-lg px-3 py-1.5 text-lg font-bold bg-indigo-50 text-indigo-700 ring-1 ring-indigo-100">
                                {{ number_format($bankTransfer->amount, 2) }}
                            </span>
                        </dd>
                    </div>
                    ...
                    <div class="pt-2">
                        ...
                        <dd class="mt-1 text-sm text-gray-700 bg-gray-50 rounded-lg p-3 border border-gray-200">{{ $bankTransfer->details }}</dd>
                    </div>
                    @endif
                </dl>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-4 sm:p-6">
                ...
                <dl class="space-y-4">
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-gray-500">Bank</dt>
                        <dd class="mt-1 text-base font-semibold text-gray-900">{{ $bankTransfer->toBank?->bank_name ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-gray-500">Currency</dt>
                        <dd class="mt-1 text-sm font-medium text-gray-900">
                            {{ $bankTransfer->toBank?->currency?->currency ?? '—' }}
                            <span class="text-gray-500">({{ $bankTransfer->toBank?->currency?->currency_symbol ?? '—' }})</span>
                        </dd>
                    </div>
                </dl>
            </div>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 mb-6 py-2 text-center sm:text-left">
        <span class="text-sm font-semibold text-rose-700 max-w-full truncate px-2" title="{{ $bankTransfer->fromBank?->bank_name ?? '—' }}">{{ $bankTransfer->fromBank?->bank_name ?? '—' }}</span>
        <span class="flex h-10 w-10 items-center justify-center rounded-full bg-sky-100 text-sky-600">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/>
            </svg>
        </span>
        <span class="text-sm font-semibold text-emerald-700 max-w-full truncate px-2" title="{{ $bankTransfer->toBank?->bank_name ?? '—' }}">{{ $bankTransfer->toBank?->bank_name ?? '—' }}</span>
    </div>

    @if($bankTransfer->attachments->isNotEmpty())
        ...
        <div class="border-b border-gray-100 bg-white px-4 sm:px-6 py-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500">{{ $bankTransfer->attachments->count() }} file(s)</p>
                </div>
            </div>
        </div>
        <div class="p-4 sm:p-6">
    ...
    <div class="flex flex-col-reverse sm:flex-row sm:flex-wrap sm:items-center sm:justify-between gap-3">
        <a href="{{ route('bank-transfers.index') }}" class="inline-flex items-center justify-center sm:justify-start text-sm font-medium text-gray-600 hover:text-gray-900 transition w-full sm:w-auto">
            <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
            </svg>
        ...
        <a href="{{ route('bank-transfers.edit', $bankTransfer) }}"
            class="inline-flex items-center justify-center w-full sm:w-auto rounded-lg bg-gradient-to-br from-indigo-600 to-slate-700 px-4 py-2.5 sm:py-2 text-sm font-semibold text-white shadow-sm hover:from-indigo-700 hover:to-slate-800 transition">
            Edit transfer
        </a>
    </div>
</x-app-layout>
