<x-app-layout>
    ...
    <div class="relative backdrop-blur-xl bg-white/70 rounded-2xl shadow-xl shadow-sky-500/5 border border-white/60 p-6 mb-6 mt-4 overflow-hidden group">
        ...
        <div class="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold bg-gradient-to-r from-gray-900 via-sky-800 to-indigo-900 bg-clip-text text-transparent">
                        Bank Transfer #{{ $bankTransfer->bank_transfer_id }}
                    </h1>
            ...
            <div class="flex flex-wrap items-center gap-2 shrink-0">
                ...
                <form action="{{ route('bank-transfers.destroy', $bankTransfer) }}" method="POST" class="inline"
        ...
        <div class="relative overflow-hidden rounded-xl shadow-lg border border-gray-200/80 backdrop-blur-sm bg-white/90">
            <div class="relative p-6">
                <div class="flex items-center gap-3 mb-5 pb-4 border-b border-gray-200">
                    ...
                    <div>
                        <h2 class="text-sm font-bold uppercase tracking-wide text-gray-800">Transfer Details</h2>
                    </div>
                </div>
                <dl class="space-y-4">
                    <div>
                    ...
                    <div class="pt-2">
                        ...
                        <dd class="mt-1 text-sm text-gray-700 bg-gray-50 rounded-lg p-3 border border-gray-100">{{ $bankTransfer->details }}</dd>
                    </div>
                    @endif
                </dl>
            </div>
    ...
    <div class="flex items-center justify-center gap-4 mb-6 py-2">
        <span class="text-sm font-semibold text-rose-700">{{ $bankTransfer->fromBank?->bank_name ?? '—' }}</span>
        <span class="flex h-10 w-10 items-center justify-center rounded-full bg-sky-100 text-sky-600">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/>
            </svg>
        </span>
        <span class="text-sm font-semibold text-emerald-700">{{ $bankTransfer->toBank?->bank_name ?? '—' }}</span>
    </div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <a href="{{ route('bank-transfers.index') }}" class="inline-flex items-center text-sm font-medium text-gray-600 hover:text-gray-900 transition">
            <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
            </svg>
