<x-app-layout>
    ...
    @php
        ...
        if ($rawOldDate && preg_match('/^\d{4}-\d{2}-\d{2}$/', $rawOldDate)) {
            try { $salesCreateDisplayDate = \Carbon\Carbon::createFromFormat('Y-m-d', $rawOldDate)->format('d/m/Y'); } catch (\Throwable $e) {}
        }
    @endphp
    ...
    <div class="bg-white shadow-sm rounded-xl border border-gray-200 mt-4">
        ...
        <div class="flex items-center justify-between px-4 sm:px-6 py-2.5 border-b border-gray-100">
            <div class="flex items-center gap-2">
                ...
                <div>
                    <h4 class="text-sm font-bold text-gray-900 leading-tight">New Sales</h4>
                    <p class="text-xs text-gray-500 mt-0.5">Record a sales voucher</p>
                </div>
            </div>
        </div>

        <form method="POST" action="{{ route('sales.store') }}" id="salesForm" class="px-4 sm:px-6 py-2 pb-3 space-y-1.5">
            @csrf
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-1.5 pb-1.5 border-b border-gray-100">
                <div>
                    ...
                    <input type="text" id="date_added" name="date_added" value="{{ $salesCreateDisplayDate }}" required readonly
                ...
                <table class="w-full border border-gray-300 text-xs rounded">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="py-1.5 px-3">
                                <input type="number" id="currency_amount" name="currency_amount" step="any" value="{{ old('currency_amount') }}" required
                ...
                <table class="w-full border border-gray-300 text-xs rounded">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="py-1.5 px-3">
                                <input type="number" id="party_amount" name="party_amount" step="any" value="{{ old('party_amount') }}" required
