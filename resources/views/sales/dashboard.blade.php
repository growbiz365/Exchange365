    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 border-l-4 border-l-emerald-500 p-5 hover:shadow-md transition-all duration-300 group">
            <div class="flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-0.5">{{ number_format($totalSales) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 border-l-4 border-l-slate-600 p-5 hover:shadow-md transition-all duration-300 group">
            <div class="flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wide">Total Currency (Bank)</p>
                    <p class="text-2xl font-bold text-gray-900 mt-0.5">@currency($totalCurrencyAmount)</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 border-l-4 border-l-rose-500 p-5 hover:shadow-md transition-all duration-300 group">
            <div class="flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-medium text-gray-500 uppercase tracking-wide">Total Party Amount</p>
                    <p class="text-2xl font-bold text-gray-900 mt-0.5">@currency($totalPartyAmount)</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-5">
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-2">
                ...
                <div>
                    <h2 class="text-sm font-bold text-gray-900">Recent Sales</h2>
                    <p class="text-xs text-gray-500">Latest sales transactions</p>
                </div>
            </div>
            <a href="{{ route('sales.index') }}"
                ...
                <table class="min-w-full divide-y divide-gray-100 text-sm">
                    <thead>
                        <tr class="bg-gray-50">
                            ...
                            <th class="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide">Currency</th>
                            <th class="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide">Party Amount</th>
                            ...
                            <tr class="hover:bg-emerald-50/40 transition-colors duration-150">
                                ...
                                <td class="px-4 py-2.5 text-sm text-right">
                                    <div class="inline-flex items-center gap-3">
                                        <a href="{{ route('sales.show', $s) }}" class="text-xs font-semibold text-emerald-700 hover:text-emerald-900 transition-colors">View</a>
                                        <a href="{{ route('sales.edit', $s) }}" class="text-xs font-semibold text-gray-500 hover:text-gray-700 transition-colors">Edit</a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
        ...
        @else
            <div class="text-center py-10">
                ...
                <p class="text-sm font-semibold text-gray-900 mb-1">No sales yet</p>
                <p class="text-xs text-gray-500">Add your first sale to get started.</p>
            </div>
        @endif
    </div>
</x-app-layout>
