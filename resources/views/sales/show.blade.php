<x-app-layout>
    @section('title', 'Sales #' . $sale->sales_id . ' - ExchangeHub')
    <x-breadcrumb :breadcrumbs="[
        ['url' => route('dashboard'), 'label' => 'Home'],
       
        ['url' => route('sales.index'), 'label' => 'Sales'],
        ['url' => '#', 'label' => 'Sales #' . $sale->sales_id]
    ]" />
        ...
        <div class="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="flex items-start gap-3 sm:space-x-4 min-w-0">
                <div class="flex-shrink-0">
                    <div class="bg-gradient-to-br from-emerald-500 to-teal-600 p-2.5 sm:p-3 rounded-xl shadow-lg transform group-hover:scale-105 transition-transform duration-300">
                ...
                <div class="min-w-0 flex-1">
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">Sales #{{ $sale->sales_id }}</h1>
                    <p class="text-xs sm:text-sm text-gray-500 mt-0.5">
                        {{ $sale->date_added->format('l, d F Y') }}
                        @if($sale->user)
                            <span class="text-gray-400"> · Recorded by {{ $sale->user->name }}</span>
                        @endif
                    </p>
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 w-full lg:w-auto lg:shrink-0">
                ...
                <form action="{{ route('sales.destroy', $sale) }}" method="POST" class="w-full sm:w-auto"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-4 sm:p-6">
                <div class="flex items-center gap-3 mb-5 pb-4 border-b border-gray-100">
                    ...
                    <div>
                        <h2 class="text-sm font-bold uppercase tracking-wide text-gray-900">Withdrawal from Bank ( جمع )</h2>
                    </div>
                </div>
                <dl class="space-y-4">
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-gray-500">Bank</dt>
                        <dd class="mt-1 text-base font-semibold text-gray-900">{{ $sale->bank?->bank_name ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-gray-500">Currency</dt>
                        <dd class="mt-1 text-sm font-medium text-gray-900">
                            {{ $sale->bank?->currency?->currency ?? '—' }}
                            <span class="text-gray-500">({{ $sale->bank?->currency?->currency_symbol ?? '—' }})</span>
                        </dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-gray-500">Currency Amount (withdrawal)</dt>
                        <dd class="mt-1">
                            <span class="inline-flex items-center rounded-lg px-3 py-1.5 text-lg font-bold bg-rose-50 text-rose-700 ring-1 ring-rose-100">
                                {{ number_format($sale->currency_amount, 2) }} {{ $sale->bank?->currency?->currency_symbol ?? '' }}
                            </span>
                        </dd>
                    </div>
                </dl>
            ...
            <div class="p-4 sm:p-6">
                <div class="flex items-center gap-3 mb-5 pb-4 border-b border-gray-100">
                    <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-emerald-50 border border-emerald-100">
                        <svg class="h-5 w-5 text-emerald-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    </div>
                    <div>
                ...
                <dl class="space-y-4">
                    <div>
                        ...
                        <dd class="mt-1 text-base font-semibold text-gray-900">{{ $sale->date_added->format('d M Y') }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-gray-500">Operation</dt>
                        <dd class="mt-1">
                            <span class="inline-flex px-2.5 py-1 text-xs font-semibold rounded-full border {{ $sale->transaction_operation === \App\Models\Sale::TRANSACTION_MULTIPLY ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-sky-50 text-sky-700 border-sky-100' }}">
                                {{ $sale->transaction_operation_label }}
                            </span>
                        </dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-gray-500">Rate</dt>
                        <dd class="mt-1 text-sm font-medium text-gray-900">{{ number_format($sale->rate, 4) }}</dd>
                    </div>
                    @if($sale->details)
                    <div class="pt-2">
                        <dt class="text-xs font-medium uppercase tracking-wider text-gray-500 mb-1">Details</dt>
                        <dd class="mt-1 text-sm text-gray-700 bg-gray-50 rounded-lg p-3 border border-gray-200">{{ $sale->details }}</dd>
                    </div>
                    @endif
                </dl>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-4 sm:p-6">
                <div class="flex items-center gap-3 mb-5 pb-4 border-b border-gray-100">
                    ...
                    <div>
                        <h2 class="text-sm font-bold uppercase tracking-wide text-gray-900">Party ( جمع )</h2>
                    </div>
                </div>
                <dl class="space-y-4">
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-gray-500">Party</dt>
                        <dd class="mt-1 text-base font-semibold text-gray-900">{{ $sale->party?->party_name ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-gray-500">Party Currency</dt>
                        <dd class="mt-1 text-sm font-medium text-gray-900">
                            {{ $sale->partyCurrency?->currency ?? '—' }}
                            <span class="text-gray-500">({{ $sale->partyCurrency?->currency_symbol ?? '—' }})</span>
                        </dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wider text-gray-500">Party Amount</dt>
                        <dd class="mt-1">
                            <span class="inline-flex items-center rounded-lg px-3 py-1.5 text-lg font-bold bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100">
                                {{ number_format($sale->party_amount, 2) }} {{ $sale->partyCurrency?->currency_symbol ?? '' }}
                            </span>
                        </dd>
                    </div>
                </dl>
    ...
    <div class="flex flex-col-reverse sm:flex-row sm:flex-wrap sm:items-center sm:justify-between gap-3">
        <a href="{{ route('sales.index') }}" class="inline-flex items-center justify-center sm:justify-start text-sm font-medium text-gray-600 hover:text-gray-900 transition w-full sm:w-auto">
            <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            Back to Sales list
        </a>
        <a href="{{ route('sales.edit', $sale) }}" class="inline-flex items-center justify-center rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 px-4 py-2.5 sm:py-2 text-sm font-semibold text-white shadow-sm hover:from-emerald-600 hover:to-teal-700 transition w-full sm:w-auto">Edit sale</a>
    </div>
</x-app-layout>
