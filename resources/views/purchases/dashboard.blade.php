        ...
        <div class="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between sm:flex-wrap">
            <div class="flex items-start sm:items-center gap-3 sm:space-x-4 min-w-0">
                ...
                <div class="min-w-0 flex-1">
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900 mb-0.5 leading-tight">Purchase Dashboard</h1>
                    <p class="text-xs sm:text-sm text-gray-500">Overview of purchase transactions and totals</p>
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @foreach($recentPurchases as $p)
                            <tr class="hover:bg-amber-50/40 transition-colors duration-150">
                                ...
                                <td class="px-4 py-2.5 text-sm text-gray-700 max-w-[8rem] sm:max-w-none truncate sm:whitespace-normal" title="{{ $p->bank?->bank_name ?? '—' }}">{{ $p->bank?->bank_name ?? '—' }}</td>
                                <td class="px-4 py-2.5 text-sm text-gray-700 max-w-[8rem] sm:max-w-none truncate sm:whitespace-normal" title="{{ $p->party?->party_name ?? '—' }}">{{ $p->party?->party_name ?? '—' }}</td>
