<x-app-layout>
    ...
    @php
        ...
        if ($rawOldDate && preg_match('/^\d{4}-\d{2}-\d{2}$/', $rawOldDate)) {
            try {
                $purchaseEditDisplayDate = \Carbon\Carbon::createFromFormat('Y-m-d', $rawOldDate)->format('d/m/Y');
            } catch (\Throwable $e) {
                // leave as is
            }
        }
    ...
    <form method="POST" action="{{ route('purchases.update', $purchase) }}" id="purchaseForm" class="space-y-3">
        @csrf
        @method('PUT')
                    ...
                    <div class="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <input type="text" id="details" name="details"
                                   value="{{ old('details', $purchase->details) }}"
                            ...
                            <div class="flex flex-wrap items-center gap-4 sm:gap-5">
                                <label class="inline-flex items-center gap-1.5 cursor-pointer">
                                    <input type="radio" name="transaction_operation" value="2"
                                           {{ old('transaction_operation', $purchase->transaction_operation) == '2' ? 'checked' : '' }} required
                                           class="border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                                    <span class="text-sm text-gray-700">Multiply</span>
                                </label>
                                <label class="inline-flex items-center gap-1.5 cursor-pointer">
                                    <input type="radio" name="transaction_operation" value="1"
                                           {{ old('transaction_operation', $purchase->transaction_operation) == '1' ? 'checked' : '' }}
                                           class="border-gray-300 text-indigo-600 focus:ring-indigo-500" />
