<x-app-layout>
    @section('title', 'Update Purchase #' . $purchase->purchase_id . ' - ExchangeHub')
    <x-breadcrumb :breadcrumbs="[
        ['url' => route('dashboard'), 'label' => 'Home'],
        ['url' => route('purchases.index'), 'label' => 'Purchase'],
        ['url' => '#', 'label' => 'Edit #' . $purchase->purchase_id]
    ]" />

    @php
        $rawOldDate = old('date_added');
        $purchaseEditDisplayDate = $rawOldDate ?: $purchase->date_added->format('d/m/Y');
        if ($rawOldDate && preg_match('/^\d{4}-\d{2}-\d{2}$/', $rawOldDate)) {
            try { $purchaseEditDisplayDate = \Carbon\Carbon::createFromFormat('Y-m-d', $rawOldDate)->format('d/m/Y'); } catch (\Throwable $e) {}
        }
    @endphp
    ...
    <div class="bg-white shadow-sm rounded-xl border border-gray-200 mt-4">
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between px-4 sm:px-6 py-2.5 border-b border-gray-100">
            <div class="flex items-center gap-2 min-w-0">
                ...
                <div class="min-w-0">
                    <h4 class="text-sm font-bold text-gray-900 leading-tight">Update Purchase #{{ $purchase->purchase_id }}</h4>
                    <p class="text-xs text-gray-500 mt-0.5">Edit purchase voucher</p>
                </div>
            </div>
        ...
        <form method="POST" action="{{ route('purchases.update', $purchase) }}" id="purchaseForm" class="px-4 sm:px-6 py-2 pb-3 space-y-1.5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-1.5 pb-1.5 border-b border-gray-100">
                <div>
                    ...
                    <input type="text" id="date_added" name="date_added" value="{{ $purchaseEditDisplayDate }}" required readonly
                ...
                <div>
                    ...
                    <input type="text" id="details" name="details" value="{{ old('details', $purchase->details) }}"
                    ...
                    <div class="flex items-center gap-4 h-9">
                        <label class="inline-flex items-center gap-1.5 cursor-pointer">
                            <input type="radio" name="transaction_operation" value="1"
                                {{ old('transaction_operation', $purchase->transaction_operation) == '1' ? 'checked' : '' }} required
                                class="border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                            <span class="text-sm font-medium text-gray-700">Divide (÷)</span>
                        </label>
                        <label class="inline-flex items-center gap-1.5 cursor-pointer">
                            <input type="radio" name="transaction_operation" value="2"
                                {{ old('transaction_operation', $purchase->transaction_operation) == '2' ? 'checked' : '' }}
                                class="border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                ...
                <div>
                    ...
                    <input type="number" id="rate" name="rate" step="any" min="0.0001" value="{{ old('rate', $purchase->rate) }}" required inputmode="decimal"
                            ...
                            <td class="py-1.5 px-3">
                                <select id="bank_id" name="bank_id" required
                                    ...
                                    @foreach($banks as $bank)
                                        <option value="{{ $bank->bank_id }}" {{ old('bank_id', $purchase->bank_id) == $bank->bank_id ? 'selected' : '' }}>
                        ...
                        <tr>
                            ...
                            <td class="py-1.5 px-3">
                                <input type="number" id="credit_amount" name="credit_amount" step="any"
                                    value="{{ old('credit_amount', $purchase->credit_amount) }}" required
                            ...
                            <td class="py-1.5 px-3">
                                <select id="party_id" name="party_id" required
                                    ...
                                    @foreach($parties as $party)
                                        <option value="{{ $party->party_id }}" {{ old('party_id', $purchase->party_id) == $party->party_id ? 'selected' : '' }}>
                                            {{ $party->party_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <td class="py-1.5 px-3">
                                <select id="party_currency_id" name="party_currency_id" required
                                    ...
                                    @foreach($currencies as $c)
                                        <option value="{{ $c->currency_id }}" {{ old('party_currency_id', $purchase->party_currency_id) == $c->currency_id ? 'selected' : '' }}>
                        ...
                        <tr>
                            ...
                            <td class="py-1.5 px-3">
                                <input type="number" id="debit_amount" name="debit_amount" step="any"
                                    value="{{ old('debit_amount', $purchase->debit_amount) }}" required
    ...
    <x-chosen-styles />
    <x-form-bold-input-styles form-id="purchaseForm" />
