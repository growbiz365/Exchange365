<x-app-layout>
    ...
    <!-- Subtle Background Gradient (ExchangeHub variant) -->
    <div class="fixed inset-0 -z-10 bg-gradient-to-br from-sky-50/40 via-white to-amber-50/30 pointer-events-none"></div>
        ...
        <div class="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
            <div class="flex items-start lg:items-center gap-3 flex-1">
                ...
                <div class="flex-1 min-w-0">
                    <h1 class="text-xl lg:text-2xl font-bold bg-gradient-to-r from-gray-900 via-sky-800 to-emerald-800 bg-clip-text text-transparent mb-0.5 leading-tight">
                        Welcome back, <span class="bg-gradient-to-r from-sky-700 to-emerald-700 bg-clip-text text-transparent">{{ Auth::user()->name }}</span>!
                    </h1>
                    <p class="text-xs text-gray-600 mb-1">Currency exchange business overview</p>
    ...
    <!-- Key Statistics Cards (compact) -->
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-4">
        <div class="relative bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-lg shadow-sm border border-emerald-100 p-3 overflow-hidden group hover:shadow-md transition-all duration-300">
            <div class="relative flex items-center gap-2">
                <div class="flex-shrink-0 w-9 h-9 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-lg flex items-center justify-center shadow-sm">
                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-semibold text-emerald-700 uppercase tracking-wide truncate">General Vouchers</p>
                    <p class="text-lg font-bold text-emerald-900 leading-tight">{{ number_format($stats['general_vouchers_amount']) }}</p>
                    <span class="text-xs text-emerald-600">{{ $stats['total_general_vouchers'] }} vouchers</span>
                </div>
            </div>
        </div>

        <div class="relative bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg shadow-sm border border-blue-100 p-3 overflow-hidden group hover:shadow-md transition-all duration-300">
            <div class="relative flex items-center gap-2">
                <div class="flex-shrink-0 w-9 h-9 bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg flex items-center justify-center shadow-sm">
                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/>
                    </svg>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-semibold text-blue-700 uppercase tracking-wide truncate">Party Transfers</p>
                    <p class="text-lg font-bold text-blue-900 leading-tight">{{ number_format($stats['party_transfers_amount']) }}</p>
                    <span class="text-xs text-blue-600">{{ $stats['total_party_transfers'] }} transfers</span>
                </div>
            </div>
        </div>

        <div class="relative bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg shadow-sm border border-purple-100 p-3 overflow-hidden group hover:shadow-md transition-all duration-300">
            <div class="relative flex items-center gap-2">
                <div class="flex-shrink-0 w-9 h-9 bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg flex items-center justify-center shadow-sm">
                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                    </svg>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-semibold text-purple-700 uppercase tracking-wide truncate">Banks</p>
                    <p class="text-lg font-bold text-purple-900 leading-tight">{{ number_format($stats['total_banks']) }}</p>
                    <span class="text-xs text-purple-600">Bank accounts</span>
                </div>
            </div>
        </div>

        <div class="relative bg-gradient-to-br from-amber-50 to-amber-100 rounded-lg shadow-sm border border-amber-100 p-3 overflow-hidden group hover:shadow-md transition-all duration-300">
            <div class="relative flex items-center gap-2">
                <div class="flex-shrink-0 w-9 h-9 bg-gradient-to-br from-amber-500 to-orange-600 rounded-lg flex items-center justify-center shadow-sm">
                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                    </svg>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-semibold text-amber-700 uppercase tracking-wide truncate">Parties</p>
                    <p class="text-lg font-bold text-amber-900 leading-tight">{{ number_format($stats['total_parties'] ?? 0) }}</p>
                    <span class="text-xs text-amber-600">Customers & suppliers</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="relative bg-gray-100 rounded-xl shadow-lg border border-gray-200 p-4 mb-4 overflow-hidden group">
        <div class="relative flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 gap-2">
            <div class="flex items-center gap-2">
                ...
                <div>
                    <h2 class="text-base font-bold text-gray-900">Exchange Shortcuts</h2>
                    <p class="text-xs text-gray-600">Frequently used features</p>
                </div>
            </div>
            <div class="flex items-center gap-1.5 px-2 py-1 bg-emerald-50 rounded-lg border border-emerald-100">
        ...
        <div class="relative grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
            <a href="{{ route('general-vouchers.create') }}"
                ...
                <div class="flex flex-col items-center text-center gap-1.5">
                    ...
                    <span class="text-xs font-semibold text-gray-900 group-hover/card:text-emerald-700">General Voucher</span>
                    <span class="text-xs text-gray-500">Create voucher</span>
                </div>
            </a>
            @can('view parties')
            <a href="{{ route('party-transfers.create') }}" class="group/card relative bg-gray-50 rounded-lg border border-gray-200 p-3 hover:shadow-md hover:border-blue-200 transition-all duration-300">
                <div class="flex flex-col items-center text-center gap-1.5">
                    ...
                    <span class="text-xs font-semibold text-gray-900 group-hover/card:text-blue-700">Party Transfer</span>
                    <span class="text-xs text-gray-500">Between parties</span>
                </div>
            </a>
            @endcan
            @can('view banks')
            <a href="{{ route('money-exchanges.create') }}" class="group/card relative bg-gray-50 rounded-lg border border-gray-200 p-3 hover:shadow-md hover:border-teal-200 transition-all duration-300">
                <div class="flex flex-col items-center text-center gap-1.5">
                    ...
                    <span class="text-xs font-semibold text-gray-900 group-hover/card:text-teal-700">Money Exchange</span>
                    <span class="text-xs text-gray-500">Between banks</span>
                </div>
            </a>
            @endcan
            @can('view parties')
            <a href="{{ route('parties.index') }}" class="group/card relative bg-gray-50 rounded-lg border border-gray-200 p-3 hover:shadow-md hover:border-purple-200 transition-all duration-300">
                <div class="flex flex-col items-center text-center gap-1.5">
                    ...
                    <span class="text-xs font-semibold text-gray-900 group-hover/card:text-purple-700">Parties</span>
                    <span class="text-xs text-gray-500">All contacts</span>
                </div>
            </a>
            @endcan
            @can('view banks')
            <a href="{{ route('banks.dashboard') }}" class="group/card relative bg-gray-50 rounded-lg border border-gray-200 p-3 hover:shadow-md hover:border-blue-200 transition-all duration-300">
                <div class="flex flex-col items-center text-center gap-1.5">
                    ...
                    <span class="text-xs font-semibold text-gray-900 group-hover/card:text-blue-700">Banks</span>
            ...
            <a href="{{ route('reports.index') }}" class="group/card relative bg-gray-50 rounded-lg border border-gray-200 p-3 hover:shadow-md hover:border-teal-200 transition-all duration-300">
                <div class="flex flex-col items-center text-center gap-1.5">
                    ...
                    <span class="text-xs font-semibold text-gray-900 group-hover/card:text-teal-700">Reports</span>
                    <span class="text-xs text-gray-500">View reports</span>
                </div>
            </a>
        </div>
    </div>
        ...
        <div class="relative bg-gray-100 rounded-xl shadow-sm border border-gray-200 p-4 overflow-hidden">
            <div class="flex items-center gap-2 mb-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500">Access reports</p>
                </div>
            </div>
        ...
        <div class="lg:col-span-2 relative bg-gray-100 rounded-xl shadow-sm border border-gray-200 p-4 overflow-hidden">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                <div class="flex items-center justify-between p-2 bg-gray-50 rounded-lg border border-gray-200 sm:col-span-2">
                    ...
                    <span class="text-sm font-bold text-purple-900">{{ $stats['total_banks'] }} · {{ $stats['total_parties'] }} · {{ $stats['total_assets'] }}</span>
                </div>
            </div>
        </div>
    </div>
