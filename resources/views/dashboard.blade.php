<x-app-layout>
    ...
    <div class="flex flex-col lg:flex-row gap-3 sm:gap-4 items-start mt-1 sm:mt-2">
        ...
        <div class="flex-1 min-w-0 space-y-4">
            ...
            <!-- Header Section -->
            <div class="relative bg-white rounded-xl shadow-sm border border-gray-200 p-4 lg:p-5 overflow-hidden group">
                ...
                <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                <h3 class="text-sm font-bold text-gray-900">Top Parties by Balance</h3>
                                <p class="text-xs text-gray-500">Highest outstanding balances</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3 text-[10px] font-semibold">
                            <span class="flex items-center gap-1"><span class="inline-block w-2.5 h-2.5 rounded-sm bg-emerald-500"></span> Receivable</span>
                            <span class="flex items-center gap-1"><span class="inline-block w-2.5 h-2.5 rounded-sm bg-rose-400"></span> Payable</span>
                        </div>
                    </div>

                    @if($topParties->isNotEmpty())
                        @php
                            $maxAbs = $topParties->max(fn($p) => abs($p->net_balance));
                        @endphp
                        <div class="space-y-2">
                            @foreach($topParties as $party)
                                @php
                                    $isPositive  = $party->net_balance >= 0;
                                    $barWidth    = $maxAbs > 0 ? round((abs($party->net_balance) / $maxAbs) * 100) : 0;
                    ...
                    @else
                        <div class="flex flex-col items-center justify-center py-8 text-center">
                            ...
                            <p class="text-xs font-medium text-gray-500">No party balances yet</p>
                            <p class="text-[10px] text-gray-400 mt-0.5">Balances will appear once transactions are recorded</p>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="w-full lg:w-56 xl:w-64 flex-shrink-0 space-y-3 order-last lg:order-none">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-1 gap-2 sm:gap-3">
                ...
                <div class="relative overflow-hidden rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-600 shadow-sm border border-emerald-400/30 p-3 sm:p-4 group hover:shadow-md transition-all duration-200">
                    ...
                    <div class="relative">
                        <div class="flex items-center justify-between mb-2">
                            <div class="w-7 h-7 sm:w-8 sm:h-8 bg-white/20 rounded-lg flex items-center justify-center backdrop-blur-sm">
                ...
                <!-- Total Debit -->
                <div class="relative overflow-hidden rounded-xl bg-gradient-to-br from-rose-500 to-rose-600 shadow-sm border border-rose-400/30 p-3 sm:p-4 group hover:shadow-md transition-all duration-200">
                    <div class="absolute -top-4 -right-4 w-20 h-20 bg-white/10 rounded-full blur-xl pointer-events-none"></div>
                    <div class="relative">
                        <div class="flex items-center justify-between mb-2">
                            <div class="w-7 h-7 sm:w-8 sm:h-8 bg-white/20 rounded-lg flex items-center justify-center backdrop-blur-sm">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-white">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <h3 class="text-xs font-bold text-gray-900 leading-none">Bank Balances</h3>
                ...
                <div class="overflow-x-auto">
                    ...
                    <table class="w-full text-xs">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-100">
                                <th class="px-4 py-2.5 text-left font-semibold text-gray-500 uppercase tracking-wider text-[10px]">Bank</th>
                                <th class="px-4 py-2.5 text-right font-semibold text-gray-500 uppercase tracking-wider text-[10px]">Balance</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-indigo-50/40 transition-colors duration-100 group/row">
                                ...
                                <td class="px-4 py-2.5 text-right">
                                    <span class="font-semibold {{ $bank->bank_balance >= 0 ? 'text-emerald-700' : 'text-rose-600' }}">
                                        {{ number_format($bank->bank_balance) }}
                                    </span>
                                    @if($bank->currency_symbol)
                                        <span class="text-gray-400 ml-0.5">{{ $bank->currency_symbol }}</span>
                                    @endif
                                </td>
                            </tr>
                            @endforeach
                    ...
                    <div class="flex flex-col items-center justify-center py-8 px-4 text-center">
                        ...
                        <p class="text-xs font-medium text-gray-500">No banks found</p>
                        <p class="text-[10px] text-gray-400 mt-0.5">Add a bank to see balances</p>
                    </div>
                    @endif
                </div>
            </div>
